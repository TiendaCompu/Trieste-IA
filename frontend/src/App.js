      ...
      <div className="container mx-auto flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <Input
              type="text"
              placeholder="Buscar matrícula, cliente, empresa..."
              value={busquedaGlobal}
              onChange={handleBusquedaChange}
              className="w-64 bg-white/90 border-blue-200 focus:border-blue-400"
              style={{marginRight: '10px'}}
            />
            <Search className="absolute right-3 top-3 h-4 w-4 text-gray-400" />
            
            {mostrandoResultados && (resultadosBusqueda.vehiculos.length > 0 || resultadosBusqueda.clientes.length > 0) && (
              <div className="absolute top-full mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg z-50 max-h-96 overflow-y-auto">
                {resultadosBusqueda.vehiculos.map((vehiculo) => (
                  <div
                    key={vehiculo.id}
                    onClick={() => seleccionarResultado(vehiculo)}
                      ...
                      <div>
                        ...
                        {vehiculo.cliente && (
                          <div className="text-xs text-gray-500">
                            {vehiculo.cliente.empresa ? `${vehiculo.cliente.empresa} - ${vehiculo.cliente.nombre}` : vehiculo.cliente.nombre}
                          </div>
                        )}
                      </div>
                ...
                {resultadosBusqueda.clientes.map((cliente) => (
                  <div
                    key={cliente.id}
                    onClick={() => {
                      // Navegar a lista de vehículos filtrada por cliente
                      navigate(`/vehiculos?cliente=${cliente.id}`);
                      setBusquedaGlobal('');
                      setMostrandoResultados(false);
                    }}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        {cliente.empresa && (
                          <div className="text-sm text-gray-600">{cliente.empresa}</div>
                        )}
                        {cliente.telefono && (
